...
// Life areas config - using icon names, rendered in component
const LIFE_AREA_CONFIG = [
  { key: 'faith', label: 'Faith', desc: 'Prayer, scripture, devotion, spiritual growth', icon: Heart, iconColor: 'text-red-500' },
  { key: 'health', label: 'Health', desc: 'Sleep, food, movement, gym, energy, rest', icon: Activity, iconColor: 'text-rose-500' },
  { key: 'career', label: 'Career', desc: 'Applications, skills, CV, interviews, professional growth', icon: Briefcase, iconColor: 'text-red-600' },
  { key: 'havilah', label: 'Business', desc: 'Business, ventures, revenue, clients, systems', icon: Building2, iconColor: 'text-rose-600' },
  { key: 'finances', label: 'Finances', desc: 'Money tracking, savings, giving, budgeting, stewardship', icon: Wallet, iconColor: 'text-red-500' },
  { key: 'relationships', label: 'Relationships', desc: 'Family, friends, community, mentorship', icon: Users, iconColor: 'text-rose-500' },
  { key: 'personalGrowth', label: 'Personal Growth', desc: 'Learning, reading, journaling, reflection, discipline', icon: Sprout, iconColor: 'text-red-700' },
]

const CADENCE_CONFIG = [
  { label: 'Morning Alignment', time: 'Configurable', desc: 'Set the day with intention. Schedule, feelings, priorities.', Icon: Sun, iconColor: 'text-red-500' },
  { label: 'Midday Correction', time: '12:00 PM', desc: 'Reset focus. Completed, blockers, what\'s slipping.', Icon: Clock, iconColor: 'text-red-600' },
  { label: 'Evening Review', time: 'Configurable', desc: 'Close the day with honesty. Wins, lessons, drift.', Icon: Moon, iconColor: 'text-rose-500' },
  { label: 'Friday Strategic Review', time: '4:30 PM', desc: 'Serious weekly review across all life areas.', Icon: TrendingUp, iconColor: 'text-red-600' },
  { label: 'Sunday Planning', time: '6:00 PM', desc: 'Review upcoming week. Priorities, deadlines, focus blocks.', Icon: Compass, iconColor: 'text-rose-600' },
]
...
export function About() {
  ...
  // Get dynamic business label from localStorage (SSR-safe)
  const businessLabel = mounted ? (localStorage.getItem('myos-business-name') || 'Business') : 'Business'
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-black via-neutral-900 to-red-950/40 p-6 md:p-12"
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          >
            <div className="w-24 h-24 md:w-28 md:h-28 rounded-3xl overflow-hidden border-4 border-red-500/30 shadow-xl shadow-red-500/10 bg-red-600/15 flex items-center justify-center relative group">
              {profilePhoto ? (
                <img
                  src={profilePhoto}
                  alt={displayName}
          ...
          >
            <div className="flex items-center justify-center gap-2 mb-1 md:mb-2">
              ...
              <span className="text-red-400 text-[10px] md:text-xs font-semibold uppercase tracking-wider">{displayOsName}</span>
            </div>
            <h1 className="text-2xl md:text-4xl font-bold text-white mb-1 md:mb-2">{displayName}</h1>

            {/* Contact details row */}
            <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3 mt-2 md:mt-3 text-neutral-400 text-[10px] md:text-xs">
              {location && (
            ...
            {/* Bio */}
            {bio && (
              <p className="text-neutral-300 text-xs md:text-base max-w-lg leading-relaxed mt-3 md:mt-4">
                {bio}
              </p>
            )}
            {!bio && !editing && (
              <p className="text-neutral-500 text-xs md:text-sm max-w-lg leading-relaxed mt-3 md:mt-4 italic">
          ...
          {editPhoto && editPhoto !== profilePhoto && (
            <Card className="border-amber-200 bg-amber-50/50">
              <CardContent className="p-3 md:p-4 flex items-center gap-3">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl overflow-hidden border border-neutral-200">
              ...
              <div className="flex flex-wrap gap-2">
                {editValues.map((value) => (
                  <Badge
                    key={value}
                    variant="secondary"
                    className="text-xs py-1 px-2.5 bg-neutral-50 text-neutral-700 border border-neutral-200 hover:bg-red-50 hover:border-red-200 hover:text-red-700 transition-colors group cursor-pointer"
          ...
          <Card className="shadow-sm bg-gradient-to-br from-neutral-50/80 to-red-50/20">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                ...
                <h2 className="text-base md:text-lg font-medium text-neutral-800">My Mission</h2>
              </div>
        ...
        <Card className="shadow-sm border-neutral-200">
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-2 mb-3 md:mb-4">
              <Star className="h-4 w-4 md:h-5 md:w-5 text-red-500" />
              <h2 className="text-base md:text-lg font-medium text-neutral-800">Core Values</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {displayValues.map((value, i) => (
                ...
                >
                  <Badge variant="secondary" className="text-xs md:text-sm py-1 px-2.5 md:py-1.5 md:px-3 bg-neutral-50 text-neutral-700 border border-neutral-200 hover:bg-neutral-100">
                    {value}
                  </Badge>
                </motion.div>
              ))}
          ...
          <Card className="shadow-sm bg-gradient-to-br from-amber-50/50 to-neutral-50/80 border-amber-200/50">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                ...
                <h2 className="text-base md:text-lg font-medium text-neutral-800">{businessName}</h2>
              </div>
              {businessDescription && (
                <p className="text-xs md:text-sm text-neutral-600 leading-relaxed">{businessDescription}</p>
              )}
            </CardContent>
          </Card>
        </motion.div>
        ...
        <Card className="shadow-sm border-neutral-200 bg-gradient-to-br from-neutral-50/80 to-red-50/20">
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-2 mb-2 md:mb-3">
              ...
              <h2 className="text-base md:text-lg font-medium text-neutral-800">The {displayOsName} Mission</h2>
            </div>
            <p className="text-xs md:text-sm text-neutral-600 leading-relaxed mb-3 md:mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
              {[
                { Icon: Target, label: 'Accountability Partner', desc: 'Keeps you honest and aligned', color: 'text-red-500' },
                { Icon: BarChart3, label: 'Strategic Advisor', desc: 'Analyzes patterns and surfaces insights', color: 'text-red-500' },
                { Icon: Zap, label: 'Chief of Staff', desc: 'Manages your daily operating cadence', color: 'text-red-500' },
              ].map((item) => (
                <div key={item.label} className="p-2.5 md:p-3 bg-white/60 rounded-xl border border-neutral-100">
                  <item.Icon className={`h-4 w-4 md:h-5 md:w-5 ${item.color}`} />
                  <p className="text-[10px] md:text-xs font-medium text-neutral-700 mt-1">{item.label}</p>
                  <p className="text-[9px] md:text-[10px] text-neutral-500">{item.desc}</p>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <motion.div {...fadeUp} transition={{ delay: 0.5 }}>
        <div className="flex items-center gap-2 mb-3 md:mb-4">
          ...
          <h2 className="text-base md:text-lg font-medium text-neutral-800">7 Life Areas</h2>
        </div>
        <motion.div
          variants={stagger}
          initial="initial"
        ...
        >
          {LIFE_AREA_CONFIG.map((area, i) => {
            const label = area.key === 'havilah' ? businessLabel : area.label
            const desc = area.key === 'havilah' && businessName ? (businessDescription || area.desc) : area.desc
            return (
              <motion.div
                key={area.key}
                variants={fadeUp}
                ...
                <Card className="shadow-sm border border-neutral-200 hover:shadow-md transition-shadow">
                  <CardContent className="p-3 md:p-4">
                    <div className="flex items-start gap-2.5 md:gap-3">
                      <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-neutral-50 flex items-center justify-center shrink-0">
                        <area.Icon className={`h-3.5 w-3.5 md:h-4 md:w-4 ${area.iconColor}`} />
                      </div>
                      <div>
                        <h3 className="text-xs md:text-sm font-medium text-neutral-800">{label}</h3>
                        <p className="text-[10px] md:text-xs text-neutral-500 mt-0.5">{desc}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-2 mb-3 md:mb-4">
              ...
              <h2 className="text-base md:text-lg font-medium text-neutral-800">Operating Cadence</h2>
            </div>
            <div className="space-y-2 md:space-y-3">
              {CADENCE_CONFIG.map((item, i) => (
                <motion.div
                  key={item.label}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.7 + i * 0.08 }}
                  className="flex items-center gap-3 md:gap-4 p-2.5 md:p-3 rounded-xl bg-neutral-50 hover:bg-neutral-100 transition-colors"
                >
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-white flex items-center justify-center shrink-0 shadow-sm">
                    <item.Icon className={`h-3.5 w-3.5 md:h-4 md:w-4 ${item.iconColor}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="text-xs md:text-sm font-medium text-neutral-800">{item.label}</span>
                    <p className="text-[10px] md:text-xs text-neutral-500 mt-0.5">{item.desc}</p>
                  </div>
                  <Badge variant="outline" className="text-[10px] md:text-xs shrink-0 whitespace-nowrap">{item.time}</Badge>
                </motion.div>
              ))}
            </div>
          </CardContent>
      ...
      >
        <div className="inline-flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 rounded-full bg-gradient-to-r from-neutral-50 to-red-50/30 border border-neutral-200">
          <Quote className="h-3 w-3 md:h-4 md:w-4 text-neutral-400" />
          <p className="text-xs md:text-sm font-medium text-neutral-600 italic">
            The life you want becomes the life you are actually living.
          </p>
        </div>
        <p className="text-[9px] md:text-[10px] text-neutral-400 mt-2 md:mt-3">
